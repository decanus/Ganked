...
namespace Ganked\Post\Queries
{
    ...
    /**
     * @covers Ganked\Post\Queries\HasBetaRequestQuery
     */
    class HasBetaRequestQueryTest extends \PHPUnit_Framework_TestCase
    {
        public function testExecuteWorks()
        {
            $accountGateway = $this->getMockBuilder(\Ganked\Skeleton\Gateways\AccountGateway::class)
                ->setMethods(['hasBetaRequest'])
            ...
            $query = new HasBetaRequestQuery($accountGateway);

            $email = new Email('test@ganked.net');

            $accountGateway
                ->expects($this->once())
                ->method('hasBetaRequest')
                ->with((string) $email)
