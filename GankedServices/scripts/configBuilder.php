...
foreach (json_decode($_ENV['VCAP_SERVICES'], true) as $serviceProvider) {
    ...
    foreach ($serviceProvider as $serviceList) {
        ...
        if ($serviceList['name'] === 'dataPool') {
            $initest .= 'cache-redisHost = "' .  $serviceList['credentials']['host'] . '"' . PHP_EOL;
            $initest .= 'cache-redisPort = "' .  $serviceList['credentials']['port'] . '"' . PHP_EOL;
            $initest .= 'cache-redisPassword = ' .  $serviceList['credentials']['password'] . PHP_EOL;
        ...
        if ($serviceList['name'] === 'mongo') {
            $initest .= 'mongoServer = "' .  $serviceList['credentials']['uri'] . '"' . PHP_EOL;
            $initest .= 'mongoDatabase = ' .  $serviceList['credentials']['database'] . PHP_EOL;
        }

        if ($serviceList['name'] === 'lol-mongo') {
            $initest .= 'lol-mongoServer = "' .  $serviceList['credentials']['uri'] . '"' . PHP_EOL;
            $initest .= 'lol-mongoDatabase = ' .  $serviceList['credentials']['database'] . PHP_EOL;
        }
    }
}
